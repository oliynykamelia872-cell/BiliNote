...
- **本地音频文件总结**：本地文件模式支持上传 MP3 / M4A / WAV / FLAC / OGG 等纯音频文件直接生成笔记。后端自动识别音频并跳过封面提取、截图与视频理解（音频无视频画面）；前端上传框可拖入选中的音频文件，选中音频后自动关闭「原片截图」与「视频理解」选项，并读取音频时长。

### Changed

- **模型配置唯一化**：界面配置的模型成为 LLM 模型身份的唯一来源。新增服务端「默认模型」配置（设置页/引导页写入 `backend/config/model_preference.json`，接口 `GET/POST /default_model`）；生成笔记、RAG 问答、文稿修订、文档 OCR 在请求未传 `provider_id`/`model_name` 时使用默认模型，且显式传入的模型必须已在该供应商 `models` 表登记（未登记 / 供应商停用 / 默认配置失效均返回可操作提示）。删除遗留的 `openai_gpt.py` / `deepseek_gpt.py` / `qwen_gpt.py` 等环境变量 GPT 类，批量脚本与 Web 前端默认值改读服务端配置。
